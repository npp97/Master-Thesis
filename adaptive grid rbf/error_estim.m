function [ P ] = error_estim( P )
    ...
    if(P.kernel_aniso > 1)
        for l = 1:P.NX
            Finterp(l) = rbf(sqrt(sqdistance(((P.XX(l,:)-P.Xmean)/P.M)',P.Tp')),P.eps)*P.c;
        end
    else
        for l = 1:P.NX
            Finterp(l) = rbf(sqrt(sqdistance(P.XX(l,:)',P.Tp')),P.eps)*P.c;
        end
    end
    ...
    figure(6)
    clf
    r_fval = ksr(log(P.Ftrue)/log(10),log(abs(P.Ftrue-Finterp))/log(10));
    plot3(r_fval.x,r_fval.f,10*ones(1,100),'k.-','LineWidth',5,'MarkerSize',25)
%     plot3(r_fval.x,r_fval.m+r_fval.s,10*ones(1,100),'r--','LineWidth',5)
%     plot3(r_fval.x,r_fval.m-r_fval.s,10*ones(1,100),'r--','LineWidth',5)
    hold on
    plot_dens2D([log(P.Ftrue)/log(10),log(abs(P.Ftrue-Finterp))/log(10)]);
    box on
    set(gcf, 'Color', 'w')
    axis square
    colorbar
    ...
    figure(7)
    clf
    r_grad = ksr(log(sqrt(sum(P.Fgradtrue.^2,2)))/log(10),log(abs(P.Ftrue-Finterp))/log(10));
    plot3(r_grad.x,r_grad.f,10*ones(1,100),'k.-','LineWidth',5)
    hold on
    plot_dens2D([log(sqrt(sum(P.Fgradtrue.^2,2)))/log(10),log(abs(P.Ftrue-Finterp))/log(10)]);
    box on
    set(gcf, 'Color', 'w')
    axis square
    colorbar
