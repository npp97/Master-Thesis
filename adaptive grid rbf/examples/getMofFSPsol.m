% function M = getMofFSPsol(P,index,y_ind,y_index,z_ind,I_hoMz,options)
...
for iy = 1: size(y_index,1)
    ...
    for iz = 1:length(z_ind)
        m{iy}(:,iz) = (P(:,J)*index(J,z_ind(iz)))./(p{iy}(:)+1e-12);
    end
    ...
    for iC = 1:size(I_hoMz,1)
        ...
        for k = 1:size(P,1)
            C{iy}(k,iC) = P(k,J)*prod(bsxfun(@minus,index(J,z_ind(I)),m{iy}(k,I)),2)/(p{iy}(k)+1e-12);
        end
    end
end
