function [ P ] = error_estim_test( P )
    ...
    hold on
    error_ellipse(cov([sum(RR<P.D0*P.adap_rstar,2),log(abs(P.Ftrue-Finterp)/P.fmax)/log(10)]),mean([sum(RR<P.D0*P.adap_rstar,2),log(abs(P.Ftrue-Finterp)/P.fmax)/log(10)]))
    ...
    for j=1:Ne;
        ...
        for k = 1 : P.pdim
            A(k*P.N+1:(k+1)*P.N,1:P.N) = gamma*drbf(P.R,P.eps,squeeze(P.DM(k,:,:)));
        end
        F = [P.F;gamma*P.DF(:)];
        ...
        Ps.error_estim = 1;
        erl(j)=CostEps_test(ee(j),1,Ps)/Ps.fmax;
    ...
    hline([],P.eps)
    plot(P.eps,CostEps_test(P.eps*mean(P.rcp),1,P)/P.fmax,'r*')
